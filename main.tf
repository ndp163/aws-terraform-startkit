module "common" {
  ...
  cloufront_key_path              = module.media_cdn.cloudfront_key_path
}
...
module "backend" {
  ...
  sns_topic_arn            = module.sns.sns_notification_topic_arn
  codebuild_build_env_vars = merge(var.backend_codebuild_build_env_vars, {
    SECRET_NAME            = module.common.secret_name
    CLOUDFRONT_KEY_PAIR_ID = var.media_cdn_cloudfront_public_key_id
  })
